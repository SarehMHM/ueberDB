...
const getSsoOidcClient = async (ssoRegion) => {
    const { SSOOIDCClient } = await Promise.resolve().then(function(){return require('./index-DoKFkicV.js')});
    ...
    const { CreateTokenCommand } = await Promise.resolve().then(function(){return require('./index-DoKFkicV.js')});
...
const resolveSSOCredentials = async ({ ssoStartUrl, ssoSession, ssoAccountId, ssoRegion, ssoRoleName, ssoClient, clientConfig, profile, logger, }) => {
    ...
    const { SSOClient, GetRoleCredentialsCommand } = await Promise.resolve().then(function(){return require('./loadSso-BV1Vw3Q2.js')});
    const sso = ssoClient ||
