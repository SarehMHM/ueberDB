...
const getSsoOidcClient = async (ssoRegion) => {
    const { SSOOIDCClient } = await Promise.resolve().then(function(){return require('./index-jzlG4LXZ.js')});
    ...
    const { CreateTokenCommand } = await Promise.resolve().then(function(){return require('./index-jzlG4LXZ.js')});
...
const resolveSSOCredentials = async ({ ssoStartUrl, ssoSession, ssoAccountId, ssoRegion, ssoRoleName, ssoClient, clientConfig, profile, logger, }) => {
    ...
    const { SSOClient, GetRoleCredentialsCommand } = await Promise.resolve().then(function(){return require('./loadSso-DQ2UZdCu.js')});
    const sso = ssoClient ||
