...
const getSsoOidcClient = async (ssoRegion) => {
    const { SSOOIDCClient } = await Promise.resolve().then(function(){return require('./index-Dp5mV-hK.js')});
    ...
    const { CreateTokenCommand } = await Promise.resolve().then(function(){return require('./index-Dp5mV-hK.js')});
...
const resolveSSOCredentials = async ({ ssoStartUrl, ssoSession, ssoAccountId, ssoRegion, ssoRoleName, ssoClient, clientConfig, profile, logger, }) => {
    ...
    const { SSOClient, GetRoleCredentialsCommand } = await Promise.resolve().then(function(){return require('./loadSso-D1969FZ_.js')});
    const sso = ssoClient ||
