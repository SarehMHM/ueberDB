...
const getSsoOidcClient = async (ssoRegion) => {
    const { SSOOIDCClient } = await Promise.resolve().then(function(){return require('./index-Beb7xFNi.js')});
    ...
    const { CreateTokenCommand } = await Promise.resolve().then(function(){return require('./index-Beb7xFNi.js')});
...
const resolveSSOCredentials = async ({ ssoStartUrl, ssoSession, ssoAccountId, ssoRegion, ssoRoleName, ssoClient, clientConfig, profile, logger, }) => {
    ...
    const { SSOClient, GetRoleCredentialsCommand } = await Promise.resolve().then(function(){return require('./loadSso-C2Ci1TVW.js')});
    const sso = ssoClient ||
